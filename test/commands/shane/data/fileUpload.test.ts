    ...
    if (!process.env.LOCALONLY) {
        ...
        it('uploads a file with a name attached to a record', async () => {
            ...
            const results = await exec(
                `sfdx shane:data:file:upload -f sfdx-project.json -n "sfdx project json file" -p ${JSON.parse(newAcct.stdout).result.id}`,
                { cwd: testProjectName }
            );
        ...
        it('uploads a file with a name attached to a record in chatter', async () => {
            const newAcct = await exec('sfdx force:data:record:create -s Account -v "Name=Acme2" --json', { cwd: testProjectName });
            const results = await exec(
                `sfdx shane:data:file:upload -f sfdx-project.json -n "sfdx project json file" -c -p ${JSON.parse(newAcct.stdout).result.id}`,
                { cwd: testProjectName }
            );
        ...
        it('fails chatter without a parentid', async () => {
            try {
                ...
                expect(JSON.parse(stripcolor(err.stdout)).message).toContain('--parentid= must also be provided when using --chatter=');
            }
        });
