...
export default class ThemeActivate extends SfdxCommand {
  ...
  public static description = 'Activate a LightningExperienceTheme via Puppeteer/Chromium headless.  Recommended: use shane:org:reuath -r to make darn sure that the domain is ready to open something';
  ...
  public async run(): Promise<any> { // tslint:disable-line:no-any
    ...
    const results = <QueryResult> await conn.query(`select id from LightningExperienceTheme where DeveloperName = '${this.flags.name}' or MasterLabel = '${this.flags.name}'`);
    ...
    await page.evaluate( localThemeId => {
      const link: HTMLElement  = Array.from(document.querySelectorAll(`tr[data-row-key-value='${localThemeId}'] td:last-of-type a`))
        .filter( a => a.querySelector('span').innerHTML === 'Activate')[0] as HTMLElement;
      link.click();
    }, themeId);
