...
export default class Upload extends SfdxCommand {
	...
	public async run(): Promise<any> { // tslint:disable-line:no-any
		...
		if (!this.flags.chatter){
			...
			const CDLReq = <CDLCreateRequest>{
				ContentDocumentId: CV.ContentDocumentId,
				LinkedEntityId: this.flags.parentId,
				ShareType: 'V'
			};

			const CDLCreateResult = <CreateResult>await conn.sobject('ContentDocumentLink').create(CDLReq);
			...
			const feedItemCreateResult = <CreateResult>await conn.sobject('FeedItem').create(feedItemRequest);
