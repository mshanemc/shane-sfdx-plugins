...
export default class PermCheck extends SfdxCommand {
  ...
  public async run(): Promise<any> { // tslint:disable-line:no-any
    ...
    if (this.flags.field) {
      ...
      withAccess = <any[]> (await conn.query(`select ParentId, Parent.Name, Parent.IsOwnedByProfile, Parent.ProfileId from FieldPermissions where ${levelTranlator(this.flags.fieldlevel)} = true and SobjectType = '${this.flags.object}' and Field = '${this.flags.object}.${this.flags.field}'`)).records;
    } else {
      // tslint:disable-next-line:no-any
      withAccess = <any[]> (await conn.query(`select ParentId, Parent.Name, Parent.IsOwnedByProfile, Parent.ProfileId from ObjectPermissions where ${levelTranlator(this.flags.objectlevel)} = true and SobjectType = '${this.flags.object}'`)).records;
    }
    ...
    if (this.flags.users) {
      ...
      const query = `select id, username, Name, Profile.Name from User where ProfileId in (${output.filter( i => i.type === 'Profile').map( profile => `'${profile.id}'`).join(',')})`;
      // tslint:disable-next-line:no-any
      const profileUsers = <any[]> (await conn.query(query)).records;
      const userOutput = profileUsers.map( user => ({userid: user.Id, name: user.Name, username: user.Username, profile: user.Profile.Name, permsets: []}));

      const psaQuery = `select AssigneeId, Assignee.Username, Assignee.Name, PermissionSet.Name from PermissionSetAssignment where PermissionSetId in (${output.filter( i => i.type === 'PermissionSet').map( ps => `'${ps.id}'`).join(',')})`;
      // tslint:disable-next-line:no-any
      ...
      this.ux.table(
        userOutput,
        ['userid', 'name', 'username', 'profile', 'permsets']
      );
      return userOutput;
    } else {
      this.ux.table(
