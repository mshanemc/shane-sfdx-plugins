...
`
  ...
  public async run(): Promise<any> { // tslint:disable-line:no-any
    ...
    const orchestrations = JSON.parse(await request.get(`${conn.instanceUrl}/services/data/v${version}/iot/orchestrations`, {
      auth
    })).orchestrations;
    ...
    do {
      const context = JSON.parse(await request.get(`${conn.instanceUrl}/services/data/v${version}/iot/contexts/${thisOrch.orchestrationContext.id}`, {
        auth
      }));
    ...
    if (this.flags.deactivate) {
      ...
      do {
        const newOrch = JSON.parse(await request.get(`${conn.instanceUrl}/services/data/v${version}/iot/orchestrations/${thisOrch.id}`, {
          auth
        }));
