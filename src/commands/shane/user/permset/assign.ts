...
export default class UserPermsetSassign extends SfdxCommand {
    ...
    public async run(): Promise<any> {
        ...
        // find the permset id
        const permset = <QueryResult>(
            await conn.query(`select id, Name from PermissionSet where Name = '${this.flags.name}' OR Label = '${this.flags.name}'`)
        );
        ...
        this.ux.log(
            `Assigned permset ${this.flags.name} (${permset.records[0].Id}) to user ${this.flags.firstname ? this.flags.firstname : ''} ${
                this.flags.lastname
            } (${user.Id})`
        );
